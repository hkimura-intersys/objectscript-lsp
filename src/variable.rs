use crate::common::{find_return_type, find_var_type_from_expression, generic_skipping_statements, get_node_children, get_string_at_byte_range, start_of_function, successful_exit};
...
pub fn build_variable_from_set_argument_rhs(
    ...
    start_of_function("No struct (Building Variable Struct)", "build_variable_from_set_argument_rhs");
    ...
    successful_exit("No struct (Building Variable Struct)", "build_variable_from_set_argument_rhs");
...
pub fn build_variable_from_argument(
    ...
    start_of_function("No struct (Building Variable Struct)", "build_variable_from_argument");
        ...
        if node.kind() == "argument_type" {
            if let Some(type_name_node) = node.named_child(1) {
                    ...
                    get_string_at_byte_range(content, type_name_node.byte_range())
                {
                    ...
                    eprintln!("Warning: failed to get string for type name node. Node is {:?}", type_name_node);
                    generic_skipping_statements("build_variable_from_argument", "Node", "Node");
                    continue;
                }
            }
            else {
            ...
            let Some(arg_content) =
                ...
                eprintln!("Warning: failed to get string for method arg value node. Node is {:?}", arg_content_node);
                generic_skipping_statements("build_variable_from_argument", "Node", "Node");
                continue;
            };
    ...
    successful_exit("No struct (Building Variable Struct)", "build_variable_from_argument");
    (
