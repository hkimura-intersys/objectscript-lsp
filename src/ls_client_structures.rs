...
// let mut capabilities = TextDocumentSyncClientCapabilities {
...
// // When receiving a position from the LSP client
// function handleGotoDefinition(params: GotoDefinitionParams) {
// const converter = new LinePositionConverter(documentText);
//
// // Convert LSP position to tree-sitter byte offset
// const byteOffset = converter.lspToTreeSitter(params.position);
//
// // Query tree-sitter at this byte offset
// const node = tree.rootNode.descendantForByteRange(byteOffset, byteOffset);
//
// // Find definition (tree-sitter returns byte offsets)
// const definitionByteOffset = findDefinition(node);
//
// // Convert back to LSP position
// const lspPosition = converter.treeSitterToLsp(definitionByteOffset);
//
...
// ) -> Option<LspPosition> {
//     // Convert LSP position to tree-sitter byte offset
//     let byte_offset = converter.lsp_to_byte(lsp_position);
//
//     // Query tree-sitter
//     let node = tree.root_node()
//         .descendant_for_byte_range(byte_offset, byte_offset)?;
//
//     // Your logic to find definition
//     let definition_node = find_definition_node(node)?;
//
//     // Convert back to LSP position
//     let definition_start = definition_node.start_byte();
//     Some(converter.byte_to_lsp(definition_start))
// }

// use lsp_types::{Position, Range};
//
// impl PositionConverter {
//     pub fn lsp_types_position_to_byte(&self, pos: &Position) -> usize {
//         self.lsp_to_byte(LspPosition {
//             line: pos.line,
//             character: pos.character,
//         })
//     }
//
//     pub fn byte_to_lsp_types_position(&self, byte: usize) -> Position {
//         let pos = self.byte_to_lsp(byte);
//         Position {
//             line: pos.line,
//             character: pos.character,
//         }
...
//
// Surrogate pairs: Emojis and rare characters that need 2 UTF-16 code units
// Combining characters: é might be one character or two (e + ́)
// Zero-width characters: Characters that exist but don't advance the position
// Line endings: \r\n vs \n can cause off-by-one errors

// Real-World Scenario
// Here's what happens when a user clicks in their editor:
//
// User clicks after an emoji in VSCode
// VSCode says: "Click at line 0, character 8 (UTF-16)"
// Your language server needs to ask tree-sitter about that position
// You convert: UTF-16 position 8 → UTF-8 byte 12
// Query tree-sitter: "What syntax node is at byte 12?"
// Tree-sitter returns: "Function definition at bytes 10-15"
// Convert back: Bytes 10-15 → UTF-16 positions 6-11
// Send to VSCode: "Highlight characters 6-11"
